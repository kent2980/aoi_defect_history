...
from src.repository import AoiRepository
from src.service import AoiService
...
class AOIView(tk.Tk):
    ...
    def __init__(self, fillColor="white", master=None):
        ...
        # リポジトリの初期化
        self.repository = AoiRepository(
            settings_path=get_config_file_path("settings.ini"),
            sqlite_dir=PROJECT_DIR,
            schedule_directory=PROJECT_DIR,
            image_root=PROJECT_DIR,
        )

        # サービスの初期化
        self.service = AoiService(self.repository)
    ...
    def create_ui(self) -> None:
        """UI要素を作成する"""
        # Kintoneクライアントの初期化
        self.repository.init_kintone_client()
        ...
        self.repository.create_sqlite_db()
    ...
    def __alert_not_directory_settings(self) -> None:
    ...
    def __before_close(self) -> None:
        ...
        if len(self.defect_list) > 0:
            ...
            self.repository.insert_defect_info_to_db_async(self.defect_list)
    ...
    def __read_smt_schedule_async(self) -> None:
        """SMTスケジュールを非同期で読み込み"""

        def _read_schedule() -> None:
            """SMTスケジュールを読み込む"""
            try:
                # ステータスバーの更新
    ...
    def create_header(self):
        ...
        self.serial_entry.bind("<Return>", self.service.on_serial_enter)
        ...
        lot_change_button = tk.Button(
            ...
            text="指図切替",
            font=("Yu Gothic UI", 8),
            command=self.service.change_lot,
        )
    ...
    def create_defect_info_area(self):
        ...
        self.defect_entry.bind(
            "<Return>", lambda event: self.service.convert_defect_name()
        )
        ...
        save_button = tk.Button(
            ...
            command=self.service.save_defect_info,
        )
        ...
        delete_button = tk.Button(
            ...
            text="削除",
            font=("Yu Gothic UI", 10),
            command=self.service.delete_defect_info,
        )
    ...
    def read_defect_list_csv(self, filepath: str) -> None:
    ...
    def __read_defect_list_from_kintone(self) -> List[DefectInfo]:
        ...
        try:
            ...
            logger.error(f"Kintone APIからのデータ取得エラー: {e}", exc_info=True)
            return []
    ...
    def next_board(self):
        ...
        # データディレクトリが有効か確認
        if not self.service.exist_data_directory():
    ...
    def export_defect_list_to_csv(self) -> bool:
        ...
        with ThreadPoolExecutor(max_workers=THREAD_POOL_MAX_WORKERS) as executor:
            ...
            try:
                future.result(timeout=CSV_EXPORT_TIMEOUT)
            except Exception as e:
                self.after(
                    0,
                    lambda: messagebox.showerror(
    ...
    def post_kintone_record_async(self, defect_list: List[DefectInfo]) -> None:
    ...
    def delete_kintone_record_async(self, record_id: str) -> None:
        """Kintoneレコードを削除"""
